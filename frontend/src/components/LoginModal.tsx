    ...
    <div className="w-full max-w-[400px] animate-fade-in">
      ...
      {mode === 'forgot-email' && (
        <div className="animate-fade-in">
          ...
          <div className="space-y-4">
            <input type="email" value={resetEmail} onChange={e => { setResetEmail(e.target.value); setError(''); }} placeholder="请输入邮箱地址" autoFocus className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#001C3D]/20 focus:border-[#001C3D] transition-all" onKeyDown={e => e.key === 'Enter' && handleSendCode()} />
            {error && <p className="text-sm text-red-500 flex items-center gap-1"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{error}</p>}
            <button onClick={handleSendCode} disabled={resetLoading || !resetEmail} className="w-full py-3.5 bg-[#001C3D] text-white text-sm font-bold rounded-lg hover:bg-[#002855] disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2">
              {resetLoading && <svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>}
              发送验证码
            </button>
            <button onClick={backToLogin} className="w-full py-2 text-sm text-slate-500 hover:text-slate-700 transition-colors">← 返回登录</button>
          </div>
        </div>
      )}
      ...
      {mode === 'forgot-code' && (
        <div className="animate-fade-in">
          ...
          <div className="space-y-4">
            ...
            <button onClick={handleVerifyCode} disabled={resetLoading || code.join('').length !== CODE_LENGTH} className="w-full py-3.5 bg-[#001C3D] text-white text-sm font-bold rounded-lg hover:bg-[#002855] disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2">
              {resetLoading && <svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>}
              验证
            </button>
            ...
            <button onClick={() => { setMode('forgot-email'); setCode(['', '', '', '', '', '']); setError(''); }} className="w-full py-2 text-sm text-slate-500 hover:text-slate-700 transition-colors">← 返回修改邮箱</button>
          </div>
        </div>
      )}
        ...
        <div className="animate-fade-in">
          ...
          <div className="space-y-4">
            <div className="relative">
              <input type={showPassword ? 'text' : 'password'} value={newPassword} onChange={e => { setNewPassword(e.target.value); setError(''); }} placeholder="新密码（至少6位）" className="w-full px-4 py-3 pr-10 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#001C3D]/20 focus:border-[#001C3D] transition-all" />
            ...
            <input type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={e => { setConfirmPassword(e.target.value); setError(''); }} placeholder="确认新密码" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#001C3D]/20 focus:border-[#001C3D] transition-all" onKeyDown={e => e.key === 'Enter' && handleResetPassword()} />
            {error && <p className="text-sm text-red-500 flex items-center gap-1"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{error}</p>}
            <button onClick={handleResetPassword} disabled={resetLoading || !newPassword || !confirmPassword} className="w-full py-3.5 bg-[#001C3D] text-white text-sm font-bold rounded-lg hover:bg-[#002855] disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2">
              {resetLoading && <svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>}
              重置密码
            </button>
          </div>
      ...
      {mode === 'forgot-success' && (
        <div className="animate-fade-in text-center">
          ...
          <button onClick={backToLogin} className="w-full py-3.5 bg-[#001C3D] text-white text-sm font-bold rounded-lg hover:bg-[#002855] transition-all">返回登录</button>
        </div>
      )}
    </div>
  );
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        {mode === 'register' && (
          <div className="space-y-1.5">
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#001C3D]/20 focus:border-[#001C3D] transition-all hover:bg-slate-100/50" placeholder="例如：Elon Musk" required />
          </div>
        )}
        <div className="space-y-1.5">
          <label className="block text-sm font-semibold text-slate-700">{mode === 'login' ? '用户名或邮箱' : '工作邮箱'}</label>
          <input type={mode === 'login' ? 'text' : 'email'} value={email} onChange={e => setEmail(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#001C3D]/20 focus:border-[#001C3D] transition-all hover:bg-slate-100/50" placeholder={mode === 'login' ? '用户名或邮箱' : 'name@company.com'} required />
        </div>
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#001C3D]/20 focus:border-[#001C3D] transition-all hover:bg-slate-100/50 pr-10" placeholder="••••••••" required />
        ...
        <button type="submit" disabled={isLoading} className="w-full py-3.5 bg-[#001C3D] text-white text-sm font-bold rounded-lg hover:bg-[#002855] focus:ring-4 focus:ring-[#001C3D]/20 transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed shadow-lg shadow-[#001C3D]/10 flex justify-center items-center gap-2">
