...
export const TaskCard: React.FC<TaskCardProps> = ({ task, onMove, onEdit, onDelete, onToggleSubtask, onAssignSubtask, onCreateFromSubtask, isSelected, dependencyType = 'none', onSelect, isDragging, dragHandleProps, draggableProps, innerRef, style, teamMembers }) => {
    ...
    <div ref={innerRef} {...draggableProps} {...dragHandleProps} style={style} id={`task-${task.id}`} onClick={() => !isDragging && onSelect?.(task.id)} className={`bg-white p-4 rounded-xl shadow-sm border group relative flex flex-col gap-3 transition-all duration-200 ${isDragging ? 'shadow-2xl rotate-2 ring-2 ring-indigo-400 z-50 opacity-90' : getBorder()}`}>
      ...
      <div className="flex justify-between items-center pt-2 border-t border-gray-50 mt-auto">
        <div className="flex items-center space-x-2">
          <div className={`w-6 h-6 rounded-full border-2 border-white flex items-center justify-center text-xs shadow-sm shrink-0 ${mainAssignee ? mainAssignee.color : 'bg-gray-100 text-gray-500'}`} title={`负责人: ${mainAssignee?.name || '待认领'}`}>{mainAssignee?.avatar || '?'}</div>
