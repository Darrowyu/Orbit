...
export const AdminPage: React.FC = () => {
  ...
  useEffect(() => { fetchStats(); fetchUsers(); fetchLoginLogs(); }, []);
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {[
                ...
                { label: '已禁用', value: stats.disabledUsers },
                { label: '团队数', value: stats.totalTeams },
                { label: '任务数', value: stats.totalTasks },
              ].map((s, i) => (
            ...
            <div className="minimal-card p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                <div>
                  <div className="text-sm text-neutral-400 mb-2">活跃率</div>
                  <div className="text-xl font-semibold text-neutral-900">{stats.totalUsers ? Math.round((stats.activeUsers / stats.totalUsers) * 100) : 0}%</div>
                  <div className="mt-2 h-1 bg-neutral-100 rounded-full overflow-hidden">
                    <div className="h-full bg-neutral-900 rounded-full transition-all duration-500" style={{ width: `${stats.totalUsers ? (stats.activeUsers / stats.totalUsers) * 100 : 0}%` }} />
                  </div>
                </div>
                <div>
                  <div className="text-sm text-neutral-400 mb-2">平均团队人数</div>
                  <div className="text-xl font-semibold text-neutral-900">{stats.totalTeams ? (stats.totalUsers / stats.totalTeams).toFixed(1) : 0}</div>
                </div>
                <div>
                  <div className="text-sm text-neutral-400 mb-2">人均任务</div>
                  <div className="text-xl font-semibold text-neutral-900">{stats.totalUsers ? (stats.totalTasks / stats.totalUsers).toFixed(1) : 0}</div>
                </div>
                <div>
                  <div className="text-sm text-neutral-400 mb-2">团队均任务</div>
                  <div className="text-xl font-semibold text-neutral-900">{stats.totalTeams ? (stats.totalTasks / stats.totalTeams).toFixed(1) : 0}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="minimal-card overflow-hidden">
            <div className="p-4 border-b border-neutral-100 flex gap-3">
              <div className="flex-1">
                <Input type="text" placeholder="搜索..." value={search} onChange={(e) => setSearch(e.target.value)} size="sm" />
              </div>
              <div className="w-28">
                <Select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} size="sm" options={[{ value: '', label: '全部' }, { value: 'active', label: '活跃' }, { value: 'disabled', label: '禁用' }]} />
              </div>
            </div>
                ...
                <tbody>
                  {users.map((u) => (
                    <tr key={u.id} className="border-b border-neutral-50 hover:bg-neutral-50/50 transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <Avatar src={u.avatar} fallback={u.avatar} size="sm" color={u.color} />
                          <div>
                            <div className="font-medium text-neutral-900 flex items-center gap-2">
                              {u.name}
                              {u.isSuperAdmin && <span className="text-[10px] bg-neutral-100 text-neutral-600 px-1.5 py-0.5 rounded">管理员</span>}
                            </div>
                      ...
                      <td className="p-4">
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${u.isActive ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'}`}>
                          {u.isActive ? '活跃' : '禁用'}
                        </span>
                      </td>
                      ...
                      <td className="p-4">
                        <div className="flex justify-center gap-1">
                          {u.id !== user?.id && (
                            <button onClick={() => handleToggleStatus(u.id)} className="p-1.5 rounded text-neutral-400 hover:text-neutral-600 hover:bg-neutral-100 transition-colors" title={u.isActive ? '禁用' : '启用'}>
                              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d={u.isActive ? "M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" : "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"} /></svg>
                            </button>
                          )}
                          <button onClick={() => openPwdModal(u.id, u.name)} className="p-1.5 rounded text-neutral-400 hover:text-neutral-600 hover:bg-neutral-100 transition-colors" title="重置密码">
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" /></svg>
                          </button>
                          {u.id !== user?.id && (
                            <>
                              <button onClick={() => handleSetAdmin(u.id, u.isSuperAdmin)} className="p-1.5 rounded text-neutral-400 hover:text-amber-600 hover:bg-amber-50 transition-colors" title={u.isSuperAdmin ? '取消管理员' : '设为管理员'}>
                              ...
                              <button onClick={() => handleDelete(u.id)} className="p-1.5 rounded text-neutral-400 hover:text-red-500 hover:bg-red-50 transition-colors" title="删除">
        ...
        {tab === 'logs' && (
          <div className="minimal-card overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {loginLogs.length === 0 ? (
                  <tr><td colSpan={4} className="p-12 text-center text-neutral-400">暂无记录</td></tr>
                ) : loginLogs.map((log) => (
                  <tr key={log.id} className="border-b border-neutral-50 hover:bg-neutral-50/50 transition-colors">
                    <td className="p-4">
                      <div className="font-medium text-neutral-900">{log.user.name}</div>
                      <div className="text-xs text-neutral-400">{log.user.email}</div>
                    </td>
                    <td className="p-4 text-neutral-500">{log.ip || '-'}</td>
                    <td className="p-4">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${log.success ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'}`}>
                        {log.success ? '成功' : '失败'}
                      </span>
                    </td>
