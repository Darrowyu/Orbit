...
export class AiController {
  ...
  @Post('generate')
  generate(@Body() body: { title: string; customPrompt?: string }): Promise<AIResponse> { return this.ai.generateTaskDetails(body.title, body.customPrompt); }
  ...
  recommendAssignee(@Body() body: { taskTitle: string; description: string; teamMembers: any[]; taskHistory: any[] }) {
    return this.ai.recommendAssignee(body.taskTitle, body.description, body.teamMembers, body.taskHistory);
  }

  @Post('detect-risks')
  detectRisks(@Body() body: { tasks: any[] }) {
